...
namespace :chewy do
  ...
  task reset: :environment do |_task, args|
    HSChewy::RakeHelper.reset(parse_classes(args.extras))
  end
  ...
  namespace :parallel do
    ...
    task deploy: :environment do |_task, args|
      ...
      processed = HSChewy::RakeHelper.upgrade(parallel: parallel)
      HSChewy::RakeHelper.sync(except: processed, parallel: parallel)
    end
  end
  ...
  namespace :journal do
    ...
    task apply: :environment do |_task, args|
      HSChewy::RakeHelper.journal_apply(parse_journal_args(args.extras))
    end
    ...
    task clean: :environment do |_task, args|
      HSChewy::RakeHelper.journal_clean(parse_journal_args(args.extras))
    end
  end
  ...
  task apply_changes_from: :environment do |_task, args|
    ...
    HSChewy::RakeHelper.subscribed_task_stats do
      params = args.extras
      ...
      if params.empty?
        ...
        HSChewy::Journal.new.apply(time, retries: (retries.to_i if retries))
      end
    end
  end
  ...
  task clean_journal: :environment do |_task, args|
    ...
    HSChewy::Journal.new.clean(args.extras.first)
  end
end
